...
EM.Total.Markers = zeros(1,length(xx)); %1-drift, 2-saccades, 0-other (rejected,blink,etc)
...
for aa = 1:length(DriftS)
    ...
    %Update Marker
    EM.Total.Markers(DriftS(aa):DriftE(aa)) = 1;
...
for aa = 1:length(SaccS)
    ...
    %Update Markers
    EM.Total.Markers(SaccS(aa):SaccE(aa)) = 2;
