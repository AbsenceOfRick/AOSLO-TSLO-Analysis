...
try
    ...
    %[~,density,xGrid,yGrid] = kde2d_trimmed(kdetrim, xy, 20,[-20, -4], [-10 6], bw);
catch errkde
...
set(gca,'fontsize',16);
xlabel('Hor. position (arc min)')
ylabel('Ver. position (arc min)')
...
try
    ...
    %MODDED FOR UNIQUE POINTS -NRB;
    [isoProb idx] = unique(isoProb);
    desiredStep = interp1(isoProb, steps(idx), cumProb, 'pchip'); %(idx) added -NRB
    figure(fh);
    delete(ch);
